...
export const News = () => {
    const [news, setNews] = useState(new Array(5))
      
    const [isLoaded, setIsLoaded] = useState(false)
    
    useEffect(() => {
        fetch("https://www.homecredit.ru/api/v1/exchange/archive/bank_exchange/filter/?date=2021-08-04&exchange_type=1").then(resp => {
            arr.forEach((pbj,index) => {
                let newsDate = new Date(arr[index].date)
                return arr[index].date = newsDate.toLocaleDateString('en-US',{year: 'numeric', month: 'long', day: 'numeric'}) + ' •'
            })
            return arr
        }).then(resp => {
            setNews(resp)
